...
/*
 * a deno port of pandoc-filter-node with a few minor additions
 */
...
export async function toJSONFilter(action: FilterActionAsync) {
  ...
  const json = new TextDecoder().decode(await Deno.readAll(Deno.stdin));
  const data = JSON.parse(json);
  const format = Deno.args.length > 2 ? Deno.args[2] : "";
  ...
  if (Array.isArray(x)) {
    ...
    for (const item of x) {
      if (isElt(item) && action.single) {
        const res = (await action.single(item, format, meta)) || item;
...
export function walkSync(
  ...
  if (Array.isArray(x)) {
    ...
    for (const item of x) {
      if (isElt(item)) {
        const res = action(item, format, meta) || item;
        if (Array.isArray(res)) {
          for (const z of res) {
...
export function attributes(attrs: {
  ...
  attrs = attrs || {};
  const ident = attrs.id || "";
  const classes = attrs.classes || [];
  const keyvals: [string, string][] = [];
...
type WrapArray<T> = T extends undefined ? []
  : //deno-lint-ignore no-explicit-any
  T extends any[] ? IsTuple<T> extends true ? T
  : [T]
  : [T];
